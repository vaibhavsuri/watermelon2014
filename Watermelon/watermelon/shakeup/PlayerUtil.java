...
public class PlayerUtil {
	...
	static double floating_point = 0.0001;
	
	public static double get_angle_between(seed p1, seed p2)
	{
...
//	
//	public static double get_angle_between(seed p1, Pair p2)
//	{
//	    double xDiff = p2.x - p1.x;
//	    double yDiff = p2.y - p1.y;
//	    return Math.atan2(yDiff, xDiff);
...
//	
	...
	public static double distancebwseed(
			  seed a, 
			  seed b) 
	{
	...
	public static boolean validSeed(seed check, ArrayList<seed> seedlist, List<Pair> treelist, double length, double width)
	{
		for (int f = 0; f < treelist.size(); f++)
			if (PlayerUtil.distance(check, treelist.get(f)) + floating_point < distotree)
				return false;

		for (int f = 0; f < seedlist.size(); f++)
			if ((PlayerUtil.distanceSeed(check, seedlist.get(f)) + floating_point < distoseed) && check!=seedlist.get(f))
				return false;
		
		if (check.x > width - distowall || check.x < distowall || check.y > length - distowall || check.y < distowall)
			return false;
